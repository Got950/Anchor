"""Build Minecraft-style gallery assets: texture.png, model.json, preview.png.

    python assets/gallery/build_gallery.py

Part 4 graded asset is gallery/01_ember_lantern/ (same folder as the rest of the pack).
"""
...
def tex_glowcap(size=16):
    """RGBA mushroom — transparent air around stem + cap."""
    C = {
        "stem": (200, 190, 150, 255),
        "stem_lo": (150, 140, 100, 255),
        "cap": (40, 160, 90, 255),
        "cap_hi": (80, 220, 130, 255),
        "spot": (240, 255, 180, 255),
        "gills": (30, 100, 60, 255),
        "glow": (160, 255, 200, 255),
    }
    im = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    p = im.load()
    for y in range(8, 15):
        ...
        p[8, y] = C["stem_lo"]
    for y in range(3, 9):
        span = 5 if y < 6 else 4
        for dx in range(-span, span + 1):
            x = 7 + dx
            if 0 <= x < size:
                p[x, y] = C["cap_hi"] if y < 5 else C["cap"]
                if abs(dx) == span:
                    p[x, y] = C["gills"]
    for x, y in ((5, 4), (9, 5), (7, 3), (11, 4), (4, 6)):
        p[x, y] = C["spot"]
    p[6, 7] = C["glow"]
    p[9, 7] = C["glow"]
    opaque = {(r, g, b) for (r, g, b, a) in im.getdata() if a > 0}
...
def tex_copper_valve(size=16):
    C = {
        "copper": (180, 100, 60),
        "copper_hi": (220, 140, 90),
        "copper_lo": (120, 60, 40),
        "patina": (60, 140, 100),
        "bolt": (80, 50, 35),
        "wheel": (200, 160, 80),
...
def tex_obsidian_reliquary(size=16):
    C = {
        "obs": (20, 12, 28),
        "obs_hi": (50, 30, 70),
        "obs_lo": (8, 4, 12),
    ...
    def paint(p, s):
        ...
        for i in range(2, 14):
            p[i, 2] = C["gold"]
            p[i, 13] = C["gold"]
            p[2, i] = C["gold_hi"] if i % 2 == 0 else C["gold"]
            p[13, i] = C["gold"]
        # central gem + runes
        p[7, 7] = C["gem"]
        p[8, 7] = C["gem"]
...
def tex_amethyst_bud(size=16):
    ...
    def paint(p, s):
        ...
        for y in range(10, 16):
            for x in range(2, 14):
                p[x, y] = C["rock"] if (x + y) % 2 else C["rock_hi"]
        # buds
        for cx, top, w in ((5, 3, 2), (8, 1, 3), (11, 4, 2)):
            for y in range(top, 11):
                for dx in range(-w, w + 1):
                    x = cx + dx
                    if 0 <= x < s:
                        t = (y - top) / max(1, 11 - top)
                        p[x, y] = C["bud_hi"] if t < 0.3 else (C["bud"] if t < 0.7 else C["bud_lo"])
        p[8, 2] = C["spark"]
        p[5, 4] = C["spark"]
...
def model(elements, texture_key="0", particle=None, texture_size=(16, 16)):
    ...
    return {
        "credit": "Craftify BlockForge style pack — same pipeline as Part 4 (Ember Lantern)",
...
def m_cobalt_frost():
    return model([
        cuboid("brick", (0, 0, 0), (16, 16, 16), uv=(0, 0, 16, 16)),
    ], "cobalt_frost")


def m_glowcap():
    return model([
        cuboid("stem", (7, 0, 7), (9, 8, 9), uv=(6, 8, 10, 16)),
        cuboid("cap", (4, 7, 4), (12, 11, 12), uv=(2, 2, 14, 10)),
        cuboid("cap_top", (5, 11, 5), (11, 12, 11), uv=(4, 0, 12, 6)),
        cuboid("spot_a", (5, 9, 5), (6, 10, 6), uv=(4, 4, 6, 6)),
        cuboid("spot_b", (10, 8, 9), (11, 9, 10), uv=(10, 4, 12, 6)),
    ], "glowcap")


def m_copper_valve():
    return model([
        cuboid("body", (4, 0, 4), (12, 8, 12), uv=(2, 6, 14, 16)),
        cuboid("neck", (6, 8, 6), (10, 11, 10), uv=(4, 2, 12, 8)),
        cuboid("wheel", (3, 11, 7), (13, 13, 9), uv=(0, 0, 16, 6)),
        cuboid("spoke", (7, 10, 5), (9, 14, 11), uv=(6, 0, 10, 10)),
        cuboid("pipe_out", (12, 2, 6), (16, 6, 10), uv=(0, 8, 8, 14)),
    ], "copper_valve")


def m_obsidian_reliquary():
    return model([
        cuboid("chest", (2, 0, 4), (14, 8, 12), uv=(0, 4, 16, 16)),
        cuboid("lid", (1.5, 8, 3.5), (14.5, 10, 12.5), uv=(0, 0, 16, 8)),
        cuboid("latch", (7, 7, 11.5), (9, 9, 13), uv=(6, 6, 10, 10)),
        cuboid("foot_fl", (2, 0, 4), (4, 1, 6), uv=(0, 14, 4, 16)),
        cuboid("foot_br", (12, 0, 10), (14, 1, 12), uv=(12, 14, 16, 16)),
        cuboid("gem", (7, 9.5, 7), (9, 11, 9), uv=(6, 4, 10, 8)),
    ], "obsidian_reliquary")


def m_amethyst_bud():
    return model([
        cuboid("rock", (2, 0, 2), (14, 3, 14), uv=(0, 12, 16, 16)),
        cuboid("bud_main", (7, 3, 7), (9, 14, 9), uv=(6, 0, 10, 14)),
        cuboid("bud_l", (4, 3, 8), (6, 11, 10), uv=(2, 2, 6, 14)),
        cuboid("bud_r", (10, 3, 6), (12, 12, 8), uv=(10, 2, 14, 14)),
        cuboid("bud_s", (8, 3, 4), (10, 9, 6), uv=(8, 4, 12, 14)),
    ], "amethyst_bud")
...
ASSETS = [
    ...
    {
        "id": "11_cobalt_frost_brick",
        "name": "Cobalt Frost Brick",
        "concept": "An icy blue brick with mortar lines and frost cracks — cold masonry for glacier builds.",
        "theme": "ice / stone",
        "geometry": "full cube brick",
        "tex": tex_cobalt_frost,
        "model": m_cobalt_frost,
        "ai_step": "Script-painted brick grid with ice highlights and crack flecks on a 16×16 sheet.",
    },
    {
        "id": "12_glowcap_mushroom",
        "name": "Glowcap Mushroom",
        "concept": "A spotted bioluminescent mushroom — organic prop for cave and forest floors.",
        "theme": "organic / plant",
        "geometry": "multi-part (stem + cap + spots)",
        "tex": tex_glowcap,
        "model": m_glowcap,
        "ai_step": "Script drew a transparent mushroom sheet and stacked stem/cap/spot cuboids.",
    },
    {
        "id": "13_copper_pressure_valve",
        "name": "Copper Pressure Valve",
        "concept": "A patinated copper valve body with hand-wheel and outlet pipe — mechanical build detail.",
        "theme": "metal / mechanical",
        "geometry": "multi-part (body, neck, wheel, spoke, pipe)",
        "tex": tex_copper_valve,
        "model": m_copper_valve,
        "ai_step": "Script painted copper/patina bands and assembled the valve cuboid stack.",
    },
    {
        "id": "14_obsidian_reliquary",
        "name": "Obsidian Reliquary",
        "concept": "A gold-trimmed obsidian chest with a central gem — dungeon loot / ceremonial container.",
        "theme": "ceremonial / dark",
        "geometry": "multi-part chest (body, lid, latch, feet, gem)",
        "tex": tex_obsidian_reliquary,
        "model": m_obsidian_reliquary,
        "ai_step": "Script painted obsidian + gold trim runes and built the reliquary cuboids.",
    },
    {
        "id": "15_amethyst_bud_cluster",
        "name": "Amethyst Bud Cluster",
        "concept": "Violet crystal buds erupting from dark rock — geode growth cluster.",
        "theme": "crystal",
        "geometry": "multi-part (rock base + 4 buds)",
        "tex": tex_amethyst_bud,
        "model": m_amethyst_bud,
        "ai_step": "Script painted bud gradients on rock and placed four crystal cuboids.",
    },
]
...
def write_readme(rows: list[dict]) -> None:
    lines = [
        "# Craftify — BlockForge style pack",
        "",
        "All Minecraft assets for this repo live here.",
        "",
        "**Part 4 (graded):** [`01_ember_lantern/`](01_ember_lantern/) — texture, Java model, "
        "isometric preview. Other `02_`–`15_` folders are the bonus style pack.",
        "",
        "In the app: **`/gallery`**. Rebuild: `python assets/gallery/build_gallery.py`",
        "",
